...
class Moxa:
	...
	def _delete_files(self, shimoda_n):
		...
		try:
			...
			tn.read_until("Moxa", 1)  # MOXA required flush
	...
	def _reboot_moxa(self):
		print ("Make comserver execuable ...\n")
		try:
			tn = telnetlib.Telnet(moxa_ip, 23, TO)
			tn.read_until("Moxa login:", TO)
			tn.write("root\n")
...
def main():
	...
	while 1:
		try:
			...
			print "Invalid input, or folder does not exist.\nShimoda number must be a number without text.\n\n" +\
			      "Press any key to return ..."
			call("pause >nul")
