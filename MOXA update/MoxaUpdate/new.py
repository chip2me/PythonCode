...
class Adapter:
	...
	def generate_adapter_list(self):
		call("getmac /v /FO csv >" + self.file_adapter) #Show names for avaiable network adapters
...
class Moxa:
	...
	def _delete_files(self, shimoda_n):
		...
		try:
			...
			tn.read_until("dummy", 0)  # MOXA required flush
			tn.write("rm comserver.c\n")
			tn.read_until("dummy", 0)  # MOXA required flush
			tn.close() # Close telnet for now ...
			time.sleep(5) #Wait 5 seconds hmmmm DEBUG
			print ("\nDEBUG, END OF TELNET \n")
		except:
	...
	def _reboot_moxa(self):
		try:
			...
			tn.read_until("dummy", 1)  # MOXA does not reboot without it
			return True
